...
# Final Writes
st_write(final_spatial, "final_spatial_shp/final_spatial.shp")
...
st_write(redlining, 'redlining_msp.geojson')
...
st_write(racial_cov, 'data/racial_cov_msp.shp')
...
plot_ly(
  ...
  fill = 'toself',
  fillcolor = 'green',
  hoverinfo = 'none',  # Removed text, no hover information
  mode = 'none',
  opacity = .7
) %>%
  layout(
    polar = list(
      radialaxis = list(
        ...
        showticklabels = TRUE,  # Ensures tick labels are displayed
  ...
  layout(font = list(color = '#FFBF00', size = 14))
